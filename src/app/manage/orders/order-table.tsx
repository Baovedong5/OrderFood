...
import OrderStatics from "./order-static";
...
export type OrderObjectByGuestID = Record<number, GetOrderResType>;
...
const OrderTable = () => {
  ...
  const orderListQuery = useGetOrderListQuery(token, {
    fromDate,
    toDate,
  });

  const updateOrderMutattion = useUpdateOrderMutation();
  ...
  useEffect(() => {
    ...
    function onNewOrder(data: GuestCreateOrdersResType) {
      const { guest } = data[0];
      toast({
