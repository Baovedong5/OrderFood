...
export default function EditOrder({
  ...
  const [selectedDish, setSelectedDish] = useState<DishListResType[0] | null>(
    null
  );
  ...
  const { data } = useGetOrderDetailQuery(token, id as number);
  ...
  useEffect(() => {
    if (data) {
      ...
      setSelectedDish(data.data.dishSnapshot);
    }
  }, [data, form]);
  ...
  const onSubmit = async (values: UpdateOrderBodyType) => {
    ...
    const result = await updateOrderMutation.mutateAsync({ token, body });
                  ...
                  <FormItem className="grid grid-cols-4 items-center justify-items-start gap-4">
                    ...
                    <div className="flex items-center col-span-2 space-x-4">
                      <Avatar className="aspect-square w-[50px] h-[50px] rounded-md object-cover">
                        <AvatarImage
                          src={`http://localhost:8080/images/dish/${selectedDish?.image}`}
                        />
